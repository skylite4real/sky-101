...
function previewPost() {
    ...
    function displayCurrentMedia() {
        previewWrapper.style.transition = 'transform 0.3s ease-out'; // Smoother transition
    ...
    function handleStart(event) {
        ...
        isSwiping = true;
        startX = event.clientX || event.touches[0].clientX; // Adjust for touch devices
    }
    ...
    function handleMove(event) {
        ...
        const currentX = event.clientX || event.touches[0].clientX; // Adjust for touch devices
        ...
        if (Math.abs(diffX) > swipeSensitivity) {
            ...
            displayCurrentMedia();
            setTimeout(() => (swipeLocked = false), 100); // Unlock after a short delay
        }
    }

    previewWrapper.addEventListener('pointerup', handleEnd);
    previewWrapper.addEventListener('pointerleave', handleEnd);
    previewWrapper.addEventListener('touchend', handleEnd); // Added for touch devices
    ...
    function handleWheel(event) {
        ...
        setTimeout(() => (swipeLocked = false), 100); // Unlock swipe after a short delay
    }
